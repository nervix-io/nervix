...
Input data often mixes tenants, users, devices, accounts, or other business groups in the same external feed. Nervix uses explicit `CREATE BRANCH` declarations to process those groups independently. A branch names the key schema, such as `CREATE BRANCH by_tenant_user BY tenant_user_branch TTL 5m`; ingestors and reingestors attach concrete key mappings with `BRANCHED BY by_tenant_user VALUES { ... }`.
