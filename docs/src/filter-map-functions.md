...
literal, optionally negated with `-` or `NOT` and cast with `AS`, such as `-1 AS I32` or
`'2026-01-01T00:00:00Z' AS DATETIME`. Each element is evaluated once, when the statement is
applied, exactly as the same expression evaluates for a message, so an element that cannot be
evaluated, such as `300 AS U8`, rejects the statement. So does an element that reads a field or
calls a function; to test a value against other fields, compare it with `=` and combine the
comparisons with `OR`.
