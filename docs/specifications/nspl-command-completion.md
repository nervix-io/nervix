...
| Transaction staging only validates | While a transaction is open, another session observes the unchanged live configuration. Invalid statement-local mutations fail without changing the staged prefix; coordinated model runs remain possible. |
