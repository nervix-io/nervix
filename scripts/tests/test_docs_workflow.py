...
class DocsWorkflowTests(unittest.TestCase):
    ...
    def test_every_referenced_documentation_image_exists(self) -> None:
        source = Path("docs/src")
        referenced = {
            (page, target)
        ...
        self.assertTrue(referenced, "no documentation page references an image")
        for page, target in sorted(referenced):
            self.assertTrue(
                (source / target).is_file(),
                f"{page.name} references missing image {target}",
            )

    def test_no_documentation_image_is_orphaned(self) -> None:
        # Images are binary artifacts regenerated by `just docs-screenshots`; one
        # that no page references is dead weight in every clone from here on.
        source = Path("docs/src")
        pages = "\n".join(page.read_text() for page in source.glob("*.md"))

        for image in sorted((source / "images").iterdir()):
            self.assertIn(
                f"images/{image.name}",
                pages,
                f"images/{image.name} is not referenced by any chapter",
            )

    def test_console_screenshots_are_regenerated_through_a_just_recipe(self) -> None:
        justfile = Path("justfile").read_text()

        self.assertIn("docs-screenshots:", justfile)
        self.assertIn('NERVIX_DOCS_SCREENSHOT_DIR="{{ justfile_directory() }}/docs/src/images"', justfile)
        self.assertIn(
            "--input tests/features/web-console/docs_screenshots.feature",
            justfile,
        )
