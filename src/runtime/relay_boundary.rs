...
mod tests {
    ...
    use nervix_models::{
        ...
        ModelName, NodeRef, ParseAsType, RelayBranching, RelayName, RemoteAckRegistration,
    ...
    fn relay_record_batches_can_be_concatenated_without_losing_metadata() {
        ...
        let concatenated =
            RelayRecordBatch::concat(vec![left, right]).expect("batches should concat");
    ...
    async fn relay_boundary_fanout_resize_preserves_existing_owner_receiver() {
        ...
        let broadcast = match (&fanout, &resized) {
            (
                RelayBoundaryFanout::Direct(original),
                RelayBoundaryFanout::Direct(resized_fanout),
            ) => {
