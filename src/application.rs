...
impl SessionServiceImpl {
                        ...
                        Ok((mut logical_start, time_rate)) => {
                            ...
                            if let DomainPace::Paced = domain.config.pace {
                                start_clock = Some((
                                    ...
                                    logical_start,
                                    time_rate.clone(),
                                    domain.start_version.checked_add(1).assured(
    ...
    async fn process_model_mutation_batch_with_transaction(
        ...
        if !mutations.is_empty() {
            ...
            let mut ownership_handoff = None;
            let (expected_schedule, prepared_schedule, planned_relocations) = if !is_noop {
                #[cfg(feature = "testing")]
                ...
                match self
                    ...
                    .await
                {
                    Ok((schedule, relocations)) => (expected_schedule, schedule, relocations),
                    Err(error) => return command_error(error),
                }
            } else {
                (None, None, 0)
