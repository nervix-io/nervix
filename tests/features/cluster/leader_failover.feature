Feature: Cluster leader failover
  ...
  Scenario Outline: Dead scheduled node primary failover promotes a live replica
    ...
    Examples:
      ...
      | router           | pass_through        | ARRAY<F32, 2> | ARRAY<F32, 1> | CREATE ROUTER pass_through FROM notifications DEFAULT TO forwarded_notifications PARAMETERIZED BY tenant_branch FLUSH EACH 100ms MAX BATCH SIZE 1MiB ON MESSAGE ERROR LOG                                                                                                                                                                                                                                                   |
