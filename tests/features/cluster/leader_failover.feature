Feature: Cluster leader failover
  ...
  Scenario Outline: Dead scheduled node primary failover promotes a live replica
    ...
    Examples:
      ...
      | inferencer       | score_model         | ARRAY<F32, 2> | ARRAY<F32, 1> | CREATE INFERENCER score_model FROM features TO scored SET scored.tenant = features.tenant, scored.score = inner_output.score UNSET features.vector BRANCHED BY by_feature_source USING RESOURCE fraud_model VERSION 1 FILE 'models/simple_score.onnx' INPUTS { "features" DENSE TENSOR<F32>[2] = features.vector } OUTPUT SCHEMA { "score" DENSE TENSOR<F32>[1] } FLUSH IMMEDIATE ON MESSAGE ERROR LOG                 |
