Feature: Cluster leader failover
  ...
  Scenario Outline: Dead scheduled node primary failover promotes a live replica
    ...
    Examples:
      ...
      | emitter          | kafka_emit          | ARRAY<F32, 2> | ARRAY<F32, 1> | CREATE EMITTER kafka_emit FROM notifications TO KAFKA kafka_main TOPIC notifications_out ENCODE USING notification_codec INHERIT ALL FLUSH EACH 100ms MAX BATCH SIZE 1MiB ON MESSAGE ERROR LOG ON GENERAL ERROR LOG                                                                                                                                                   |
