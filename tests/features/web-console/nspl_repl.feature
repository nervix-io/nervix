Feature: Web console NSPL REPL
  ...
  Scenario: Web console submits explicit NSPL transactions
    ...
    When the web console is opened on the leader node
    Then selector ".topbar-status .pill.ok" contains "CONNECTED"
    When selector ".prompt-row input" is filled with "CREATE DOMAIN production;"
    And selector ".prompt-row input" is pressed with "Enter"
    Then selector ".terminal" contains "created domain 'production'"
    And selector ".prompt-row" contains "production"
    When selector ".prompt-row input" is filled with "BEGIN; CREATE CLIENT http_main TYPE HTTP CONFIG { 'url' = 'http://example.com/a;b' }; CREATE SCHEMA notification ( user_id I64 ); COMMIT"
    And selector ".prompt-row input" is pressed with "Enter"
    Then selector ".terminal" contains "BEGIN; CREATE CLIENT http_main TYPE HTTP CONFIG { 'url' = 'http://example.com/a;b' }; CREATE SCHEMA notification ( user_id I64 ); COMMIT"
  ...
  Scenario: Web console transactions opened on a follower are handled by the leader
    ...
    Then selector ".terminal" contains "created domain 'production'"
    And selector ".prompt-row" contains "production"
    When selector ".prompt-row input" is filled with "BEGIN; CREATE SCHEMA follower_notification ( user_id I64 ); COMMIT"
    And selector ".prompt-row input" is pressed with "Enter"
    Then selector ".terminal" contains "quiesce level: DYNAMIC"
  ...
  Scenario: Web console renders a branched correlator graph without overlaying the REPL
    ...
    When these NSPL commands are executed on the leader node
      ...
      CREATE CORRELATOR correlate_profiles
        ...
        FLUSH IMMEDIATE
        ON MESSAGE ERROR SEND TO correlator_errors
  ...
  Scenario: Web console graph processor actions describe deduplicators and reorderers
    ...
    When these NSPL commands are executed on the leader node
      ...
      CREATE DEDUPLICATOR route_notifications FROM notifications DEDUPLICATE ON input.user_id MAX TIME 10m UNBRANCHED TO priority_notifications INHERIT ALL WHERE user_id = 1 FLUSH EACH 100ms MAX BATCH SIZE 1MiB ON MESSAGE ERROR LOG TO default_notifications INHERIT ALL FLUSH EACH 100ms MAX BATCH SIZE 1MiB ON MESSAGE ERROR LOG;
      CREATE REORDERER order_notifications FROM default_notifications BY input.user_id MAX TIME 10s UNBRANCHED TO ordered_notifications INHERIT ALL FLUSH EACH 100ms MAX BATCH SIZE 1MiB ON MESSAGE ERROR LOG;
      """
    And the web console is opened on the leader node
    Then selector ".topbar-status .pill.ok" contains "CONNECTED"
  ...
  Scenario: Web console lays out datalake-style source edges and long nodes
    ...
    When these NSPL commands are executed on the leader node
      ...
      CREATE CLIENT lakehouse_catalog TYPE ICEBERG_REST CONFIG {
        ...
        SET tenant_id = message.tenant_id, device_id = message.device_id
      ...
      CREATE DEDUPLICATOR connect_without_authorization FROM auth_activity_landing
        DEDUPLICATE ON input.tenant_id, input.device_id, input.event_type, input.value
        MAX TIME 30m
        BRANCHED BY by_iot_device_activity
        TO security_events
        INHERIT ALL
        FLUSH EACH 250ms MAX BATCH SIZE 512kb
        ON MESSAGE ERROR LOG;
      CREATE DEDUPLICATOR connection_distance_alert_mapper FROM device_activity_landing
        DEDUPLICATE ON input.tenant_id, input.device_id, input.event_type, input.value
        MAX TIME 30m
        BRANCHED BY by_iot_device_activity
        ...
        FLUSH EACH 250ms MAX BATCH SIZE 512kb
        ON MESSAGE ERROR LOG;
      CREATE DEDUPLICATOR location_distance_alert_mapper FROM device_activity_landing
        DEDUPLICATE ON input.tenant_id, input.device_id, input.event_type, input.value
        MAX TIME 30m
        BRANCHED BY by_iot_device_activity
        ...
        FLUSH EACH 250ms MAX BATCH SIZE 512kb
        ON MESSAGE ERROR LOG;
      CREATE DEDUPLICATOR edge_location_lookup FROM edge_activity_landing
        DEDUPLICATE ON input.tenant_id, input.device_id, input.event_type, input.value
        MAX TIME 30m
        BRANCHED BY by_iot_device_activity
        ...
        FLUSH EACH 250ms MAX BATCH SIZE 512kb
        ON MESSAGE ERROR LOG;
      CREATE DEDUPLICATOR edge_activity_splitter FROM edge_activity_enriched_landing
        DEDUPLICATE ON input.tenant_id, input.device_id, input.event_type, input.value
        MAX TIME 30m
        BRANCHED BY by_iot_device_activity
        ...
        FLUSH EACH 250ms MAX BATCH SIZE 512kb
        ON MESSAGE ERROR LOG;
      CREATE EMITTER kafka_security_events FROM security_events TO KAFKA kafka_auth TOPIC datalake_security_events MODE NO_ACK RETRY POLICY BACKOFF 250ms MAX 30s ENCODE USING activity_codec
        INHERIT ALL
        FLUSH EACH 1s MAX BATCH SIZE 1MiB
        ON MESSAGE ERROR LOG
        ON GENERAL ERROR LOG;
      CREATE DETACHED EMITTER iceberg_connected_sessions FROM connected_sessions TO ICEBERG ON S3 s3_lakehouse TABLE datalake_connected_sessions VALUES { 'tenant_id' = input.tenant_id, 'device_id' = input.device_id, 'event_type' = input.event_type, 'value' = input.value } LOCATION 's3://nervix-iceberg/tables/datalake_connected_sessions' CATALOG lakehouse_catalog COMMIT EACH 1m MAX SIZE 512MiB MODE ACK RETRY POLICY BACKOFF 250ms MAX 30s FLUSH EACH 10s MAX BATCH SIZE 8MiB
        ON MESSAGE ERROR LOG
        ON GENERAL ERROR LOG;
      """
    And the web console is opened on the leader node
  ...
  Scenario: Web console renders branch groups across a reingestor
    ...
    When these NSPL commands are executed on the leader node
      ...
      CREATE DEDUPLICATOR notification_forwarder FROM notifications
        ...
        FLUSH EACH 100ms MAX BATCH SIZE 1MiB
        ON MESSAGE ERROR LOG;
      CREATE REINGESTOR reingestor_metrics_node FROM validated_notifications
  ...
  Scenario: Web console routes shared sink client edges around downstream branch groups
    ...
    When these NSPL commands are executed on the leader node
      ...
      CREATE CLIENT redis_alerts TYPE REDIS CONFIG { 'addr' = 'redis://127.0.0.1:6379/' }; CREATE INGESTOR http_telemetry
        ...
        SET site = message.site
        FLUSH EACH 100ms MAX BATCH SIZE 1MiB
        ON MESSAGE ERROR LOG
        ON GENERAL ERROR LOG;
      CREATE DEDUPLICATOR quality_gate FROM telemetry_by_site
        DEDUPLICATE ON input.site, input.device_id
        MAX TIME 10m
        BRANCHED BY by_http_telemetry
        TO battery_alerts
        INHERIT ALL
        ...
        INHERIT ALL
        FLUSH EACH 100ms MAX BATCH SIZE 1MiB
        ON MESSAGE ERROR LOG; CREATE REINGESTOR device_repartition FROM telemetry_clean
        TO telemetry_by_device
        INHERIT ALL
        BRANCHED BY by_device_repartition
        SET device_id = message.device_id
        FLUSH EACH 100ms MAX BATCH SIZE 1MiB
        ON MESSAGE ERROR LOG;
      CREATE DEDUPLICATOR anomaly_splitter FROM telemetry_by_device
        DEDUPLICATE ON input.device_id
        MAX TIME 10m
        BRANCHED BY by_device_repartition
        TO maintenance_alerts
        INHERIT ALL
        WHERE value >= warn_high
        FLUSH EACH 100ms MAX BATCH SIZE 1MiB
        ON MESSAGE ERROR LOG
        TO normal_telemetry
        INHERIT ALL
        FLUSH EACH 100ms MAX BATCH SIZE 1MiB
        ON MESSAGE ERROR LOG;
      CREATE EMITTER redis_battery_alerts FROM battery_alerts TO REDIS PUBSUB redis_alerts CHANNEL battery_alerts MODE NO_ACK RETRY POLICY BACKOFF 250ms MAX 30s ENCODE USING telemetry_codec
        INHERIT ALL
        FLUSH EACH 100ms MAX BATCH SIZE 1MiB
        ON MESSAGE ERROR LOG
        ON GENERAL ERROR LOG;
      CREATE EMITTER redis_maintenance_alerts FROM maintenance_alerts TO REDIS PUBSUB redis_alerts CHANNEL maintenance_alerts MODE NO_ACK RETRY POLICY BACKOFF 250ms MAX 30s ENCODE USING telemetry_codec
        INHERIT ALL
        FLUSH EACH 100ms MAX BATCH SIZE 1MiB
        ON MESSAGE ERROR LOG
        ON GENERAL ERROR LOG;
