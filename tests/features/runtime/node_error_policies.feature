Feature: Runtime node error policies
  Scenario Outline: Runtime nodes require their mandatory error policy blocks
    ...
    Examples:
      ...
      | inferencer       | CREATE INFERENCER score_model FROM features TO scored SET scored.tenant = features.tenant UNBRANCHED USING RESOURCE fraud_model VERSION 3 FILE 'models/fraud.onnx' INPUTS { "features" DENSE TENSOR<F32>[2] = features.vector } OUTPUTS { "score" DENSE TENSOR<F32>[1] = scored.score } FLUSH IMMEDIATE                                                                                                                                                                         |
