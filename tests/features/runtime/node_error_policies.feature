Feature: Runtime node error policies
  Scenario Outline: Runtime nodes require their mandatory error policy blocks
    ...
    Examples:
      | node             | statement                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
      | ingestor         | CREATE IF NOT EXISTS SCHEMA user_id_branch ( user_id I64 ); CREATE IF NOT EXISTS SCHEMA tenant_branch ( tenant STRING ); CREATE IF NOT EXISTS BRANCH by_http_notifications BY user_id_branch TTL 5m; CREATE INGESTOR http_notifications TO notifications DECODE USING notification_codec BRANCHED BY by_http_notifications VALUES { user_id = notifications.user_id } FLUSH EACH 100ms MAX BATCH SIZE 1MiB FROM ENDPOINT http_notifications_endpoint MODE NO_ACK SEQUENTIAL |
      | reingestor       | CREATE IF NOT EXISTS BRANCH by_repartition BY tenant_branch TTL 5m; CREATE REINGESTOR repartition FROM notifications TO tenant_notifications BRANCHED BY by_repartition VALUES { tenant = tenant_notifications.tenant } FLUSH EACH 100ms MAX BATCH SIZE 1MiB                                                                                                                                                                                                                |
  ...
  Scenario Outline: Pure runtime processors reject general error policies
    ...
    Examples:
      | node             | statement                                                                                                                                                                                                                                                                                                                                                           |
      | reingestor       | CREATE IF NOT EXISTS SCHEMA tenant_branch ( tenant STRING ); CREATE IF NOT EXISTS BRANCH by_repartition BY tenant_branch TTL 5m; CREATE REINGESTOR repartition FROM notifications TO tenant_notifications BRANCHED BY by_repartition VALUES { tenant = tenant_notifications.tenant } FLUSH EACH 100ms MAX BATCH SIZE 1MiB ON MESSAGE ERROR LOG ON GENERAL ERROR LOG |
