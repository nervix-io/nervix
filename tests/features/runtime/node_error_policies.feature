Feature: Runtime node error policies
  Scenario Outline: Runtime nodes require their mandatory error policy blocks
    ...
    Examples:
      ...
      | junction         | CREATE JUNCTION join_streams FROM notifications_a, notifications_b TO notifications_all UNPARAMETERIZED FLUSH EACH 100ms MAX BATCH SIZE 1MiB                                                                                                                                                                                                                                                         |
      ...
      | window processor | CREATE WINDOW PROCESSOR latency_window FROM metrics TO metric_summaries UNPARAMETERIZED WIDTH 10s DURATION STEP 5s DURATION AGGREGATE metric_summaries.tenant = FIRST(metrics.tenant), metric_summaries.total_latency = SUM(metrics.latency)                                                                                                                                                         |
      ...
      | inferencer       | CREATE INFERENCER score_model FROM features TO scored SET scored.tenant = features.tenant UNPARAMETERIZED USING RESOURCE fraud_model VERSION 3 FILE 'models/fraud.onnx' INPUTS { "features" = features.vector } OUTPUTS { "score" = scored.score } FLUSH IMMEDIATE                                                                                                                                   |
  ...
  Scenario Outline: Pure runtime processors reject general error policies
    ...
    Examples:
      ...
      | junction         | CREATE JUNCTION join_streams FROM notifications_a, notifications_b TO notifications_all UNPARAMETERIZED FLUSH EACH 100ms MAX BATCH SIZE 1MiB ON MESSAGE ERROR LOG ON GENERAL ERROR LOG                                                                                                                       |
      ...
      | window processor | CREATE WINDOW PROCESSOR latency_window FROM metrics TO metric_summaries UNPARAMETERIZED WIDTH 10s DURATION STEP 5s DURATION AGGREGATE metric_summaries.tenant = FIRST(metrics.tenant), metric_summaries.total_latency = SUM(metrics.latency) ON MESSAGE ERROR LOG ON GENERAL ERROR LOG                       |
      ...
      | inferencer       | CREATE INFERENCER score_model FROM features TO scored SET scored.tenant = features.tenant UNPARAMETERIZED USING RESOURCE fraud_model VERSION 3 FILE 'models/fraud.onnx' INPUTS { "features" = features.vector } OUTPUTS { "score" = scored.score } FLUSH IMMEDIATE ON MESSAGE ERROR LOG ON GENERAL ERROR LOG |
