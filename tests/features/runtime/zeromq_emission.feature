Feature: ZeroMQ emission
  ...
  Scenario Outline: ZeroMQ emitter filter-map reads materialized relay state
    ...
    When these NSPL commands are executed
      ...
      CREATE IF NOT EXISTS SCHEMA user_id_branch ( user_id I64 ); CREATE IF NOT EXISTS BRANCH by_notifications_source PARAMETERIZED BY user_id_branch VALUES { user_id = notifications.user_id } TTL 5m; CREATE INGESTOR notifications_source
        TO notifications
        DECODE USING notification_codec
        BRANCHED BY by_notifications_source
        FLUSH EACH 100ms MAX BATCH SIZE 1MiB
