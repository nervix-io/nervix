Feature: HTTP endpoint ingestion
  ...
  Scenario Outline: HTTPS endpoint ingestor delivers a single JSON payload to a subscribed relay
    ...
    And these NSPL commands are executed
      ...
      CREATE IF NOT EXISTS BRANCH by_http_notifications SCHEMA user_id_branch TTL 5m;
      CREATE RELAY notifications SCHEMA notification BRANCHED BY by_http_notifications;
      CREATE VHOST edge http-{{test_id}}.example.com WITH TLS tls_bundle VERSION 1;
      CREATE ENDPOINT http_notifications_endpoint
        ON edge
        PATH '/ingest'
        TYPE HTTP;
  ...
  Scenario Outline: HTTP endpoint stops routing while its domain is stopped and routes again after it restarts
    Given runtime replication is configured with replica count <replica_count> and snapshot interval "100ms"
    And a <cluster_size> node nervix cluster is started
    And the leader node is configured with these NSPL commands
      """
    ...
    When these NSPL commands are executed on the leader node
      ...
      STOP;
      """
    Then node "node-1" eventually reports status containing "{{domain}} status=Stopped"
    When http payload is posted to host "http-{{test_id}}.example.com" path "/ingest" and is not routed
      """
      {"user_id":43}
      """
