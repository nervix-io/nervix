...
//!
//! - **Owns.** The Syslog transport a client configures, its UDP, TCP and TLS senders, RFC 6587
//!   stream framing, and the payload limits each transport enforces.
//! - **Depends on.** The connector contract, vocabulary values, `error-stack`, Tokio, `rustls` and
//!   `tokio-rustls`.
