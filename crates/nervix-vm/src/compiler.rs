...
fn evaluate_set_element(element: &SpannedExpr) -> Result<TypedArray, SetElementDefect> {
    match &element.inner {
        ...
        Expr::Cast {
            ...
            match cast_constant(value, target) {
                Ok(converted) => Ok(converted),
                // A tolerant conversion yields a typed null for a value it cannot convert, and no
                // value equals a null element.
                Err(reason) => match on_failure {
                    CastFailure::Error => Err(SetElementDefect::Unevaluable(reason)),
                    CastFailure::Null => Err(SetElementDefect::Null),
                },
            }
        }
        Expr::FieldRef(_)
