...
use nervix_vm::{
    CompileBinding, CompileOptions, CompiledProgram, OutputMode, TypedArray, TypedBatch,
...
fn compile_key_projection() -> Arc<CompiledProgram> {
    let program = parse_program(
        "SET input.key_0 = input.tenant, input.key_1 = input.sequence, input.key_2 = \
...
fn compile_window_aggregate_input() -> Arc<CompiledProgram> {
    let program = parse_program(
        "SET window_input.demand_0 = input.amount, window_input.demand_1 = input.sequence, \
         window_input.demand_2 = input.amount * 2.0;",
    )
