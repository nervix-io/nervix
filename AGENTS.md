...
9. Branch parameterization is explicit and must be preserved:
   ...
   - Every other processing node (`DEDUPLICATOR`, `ROUTER`, `FORWARDER`, `UNIFIER`, `WINDOW PROCESSOR`, and future processing nodes unless explicitly promoted to this list) must execute inside one concrete branch at a time.
   ...
   - `DEDUPLICATOR`, `ROUTER`, `FORWARDER`, `UNIFIER`, and `WINDOW PROCESSOR` must preserve the branch parameterization they receive.
...
- When code relies on sorted vectors/arrays as an invariant, prefer the `sorted_vec` crate (`SortedVec` / `SortedSet`) over a plain `Vec` plus manual `sort`/`dedup`.
