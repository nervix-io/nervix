...
9. Branch parameterization is explicit and must be preserved:
   ...
   - Every other processing node (`DEDUPLICATOR`, `ROUTER`, `UNIFIER`, `WINDOW PROCESSOR`, and future processing nodes unless explicitly promoted to this list) must execute inside one concrete branch at a time.
   ...
   - `DEDUPLICATOR`, `ROUTER`, `UNIFIER`, and `WINDOW PROCESSOR` must preserve the branch parameterization they receive.
